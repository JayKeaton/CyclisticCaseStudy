...
differently?\
Lets break this business task into sub tasks:
...
-   [Cyclistic's first party
    ...
    fictional company. The data is provided by Motivate International
...
-   [Chicago ZIP
    ...
    Contains a map (shapefile) that represents the Chicago divided by
    ZIP areas

```{r setup}
dataPath <- 'C:/Users/Propriétaire/Documents/Obsidian Vault/NLP/Data_Analyst_Google_Certificate/Part8of8_Google_Analytic_Capstone/CaseStudy1_Cyclistic/data/unzip'
...
-   added **month**
-   added Zip **zoneIds** for the trips *starting location* and *ending
    location*
-   **ordered** by *station_name* then by *started_at*
...
ggplot(processedData[member_casual=="casual"], aes(x=tripduration, y =  stat(count / sum(count))))+
...
Casuals use the service all along the day, with a cliff around 5:30 and
a **peak around 17:30**\
Member's graph is similar casual's graph with two variations:

-   another **peaks around 8:00**
-   less usage during work hours
...
*Members* follow the same trend but they are much more **constant** on
their usage with only December under 4% usage, and no month above 12.5%
usage.
...
Those tendencies remain the same during the whole week, but get
reinforced for Streeterville during the weekend.\
...
The *"casual" offers* regroup single-ride and full day pass. This offer
is about **short term engagement.**\
The *"member" offer* contain annual membership. This offer is about
...
"casual" clients** while being **unattractive to "member"** client since
we don't want to convert members into casuals. Based on those
reflections and the previously seen data, here is 3 recommendations:
...
processedData %>% 
  ...
  ggplot(data = .) +
    ...
    theme(axis.text.x = element_text(angle = 45)) %>% 
  labs(title = "Chicago zone difference between bike take and bike leave")  
```
